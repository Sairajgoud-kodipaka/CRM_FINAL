...
export default function SalesAppointmentsPage() {
  ...
  const fetchAppointments = async () => {
    try {
      setLoading(true);
      console.log('🔍 [SALES APPOINTMENTS] Fetching appointments with params:', {
      ...
      console.log('📊 [SALES APPOINTMENTS] API Response:', response);
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="opacity-50 rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
      </div>
    );
  }
