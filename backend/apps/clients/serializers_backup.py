...
class ClientSerializer(serializers.ModelSerializer):
                ...
                try:
                    ...
                    for i, interest_data in enumerate(customer_interests_data):
                        try:
                            ...
                            # If no category is provided, use the product_type from the customer data
                            if not category or category == 'None' or category == 'null':
                                category = validated_data.get('product_type', 'General Jewellery')
                                print(f"No category provided, using product_type: '{category}'")
                                        ...
                                        try:
                                            ...
                                            try:
                                                ...
                                                if created:
                                                    ...
                                                    # Auto-create/update pipeline based on design selection
                                                    self._handle_pipeline_auto_creation(
                                                        result, 
                                                        preferences.get('designSelected', False),
                                                        product_name,
                                                        revenue_value,
                                                        notes
                                                    )
                    ...
                    print(f"[SUCCESS] Customer interests processing completed for client {result.id}")
                    
                except Exception as e:
                    print(f"[ERROR] Error in customer interests processing: {e}")
                    import traceback
                    print(f"Customer interests processing traceback: {traceback.format_exc()}")
            else:
                print(f"=== NO CUSTOMER INTERESTS TO PROCESS ===")
                print(f"customer_interests_data: {customer_interests_data}")
                print(f"Type: {type(customer_interests_data)}")
                print(f"Length: {len(customer_interests_data) if customer_interests_data else 'None'}")
        ...
        if customer_interests_data:
            ...
            for i, interest_data in enumerate(customer_interests_data):
                try:
                    ...
                    # If no category is provided, use the product_type from the customer data
                    if not category or category == 'None' or category == 'null':
                        category = result.product_type or 'General Jewellery'
                        print(f"No category provided, using product_type: '{category}'")
    ...
    def _handle_pipeline_auto_creation(self, client, design_selected, product_name, revenue_value, notes):
        """
        Automatically create or update SalesPipeline based on design selection.
        - If design_selected is True: Move to 'closed_won' stage
        - Otherwise: Move to 'interested' stage
        """
        try:
            ...
            # If no user context, try to get the first user for this tenant
            if not user:
                user = User.objects.filter(tenant=client.tenant).first()
            
            if not user:
                print(f"[ERROR] No user found for pipeline creation for client {client.id}")
                return
            
            # Determine pipeline stage based on design selection
            if design_selected:
                stage = SalesPipeline.Stage.CLOSED_WON
                probability = 100  # 100% probability for closed won
                pipeline_title = f"{client.full_name} - {product_name} (Design Selected)"
                pipeline_notes = f"Customer has selected design for {product_name}. {notes}"
                next_action = "Process order and arrange delivery"
            else:
                stage = SalesPipeline.Stage.INTERESTED
                probability = 50  # 50% probability for interested
                pipeline_title = f"{client.full_name} - {product_name} (Interested)"
                pipeline_notes = f"Customer is interested in {product_name}. {notes}"
                next_action = "Follow up and present design options"
            
            # Check if pipeline already exists for this client and product
            existing_pipeline = SalesPipeline.objects.filter(
                client=client,
            ...
            if existing_pipeline:
                ...
                existing_pipeline.stage = stage
                existing_pipeline.probability = probability
                existing_pipeline.expected_value = revenue_value
                existing_pipeline.notes = pipeline_notes
                existing_pipeline.next_action = next_action
                existing_pipeline.save()
                
                print(f"[SUCCESS] Updated existing pipeline for {client.full_name}: {stage} (₹{revenue_value})")
            else:
                # Create new pipeline
                pipeline = SalesPipeline.objects.create(
                    ...
                    expected_value=revenue_value,
                    notes=pipeline_notes,
                    next_action=next_action,
                    tenant=client.tenant
                )
                
                print(f"[SUCCESS] Created new pipeline for {client.full_name}: {stage} (₹{revenue_value})")
                
        except Exception as e:
            print(f"[ERROR] Error handling pipeline auto-creation: {e}")
            import traceback
            print(f"Pipeline creation traceback: {traceback.format_exc()}")
