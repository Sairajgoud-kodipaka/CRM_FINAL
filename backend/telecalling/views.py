...
class SimpleCallLogViewSet(viewsets.ModelViewSet):
    """Simple call log management for manual call tracking"""
    queryset = SimpleCallLog.objects.all()
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get_queryset(self):
        ...
        if user.role == 'tele_calling':
            # Telecallers see only their call logs
            return SimpleCallLog.objects.filter(telecaller=user)
        elif user.role in ['manager', 'business_admin', 'platform_admin']:
            # Managers and admins see all call logs
            return SimpleCallLog.objects.all()
        return SimpleCallLog.objects.none()
    
    def get_serializer_class(self):
        from .serializers import SimpleCallLogSerializer
        return SimpleCallLogSerializer
    
    def perform_create(self, serializer):
        serializer.save(telecaller=self.request.user)
    ...
    def my_calls(self, request):
        ...
        call_logs = self.get_queryset()
        
        # Apply filters
        status_filter = request.query_params.get('status')
        if status_filter and status_filter != 'all':
            call_logs = call_logs.filter(call_status=status_filter)
        ...
        call_logs_page = call_logs[start:end]
        
        serializer = self.get_serializer(call_logs_page, many=True)
        ...
        return Response({
            ...
            'page': page,
            'page_size': page_size,
            'total_pages': (call_logs.count() + page_size - 1) // page_size
        })


class GoogleSheetsCustomerNamesView(APIView):
    """Get customer names from Google Sheets for dropdown"""
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get(self, request):
        try:
            ...
            sheet_data = google_sheets_service.get_sheet_data("Sheet1!A:J")
            
            if not sheet_data or len(sheet_data) < 2:
                return Response({
                    'error': 'No customer data found in Google Sheets'
                }, status=status.HTTP_404_NOT_FOUND)
            
            headers = sheet_data[0]
            customer_names = []
            
            # Process each row to extract customer names
            for row in sheet_data[1:]:
                ...
                # Extract customer name with flexible column matching
                name = (
                ...
                if name and name.strip():
                    customer_names.append({
                        'id': f"sheet_{hash(name + phone)}",
                        'name': name.strip(),
                        'phone': phone or '',
                        'source': source or 'google_sheets'
                    })
            ...
            return Response(unique_customers)
            
        except Exception as e:
            logger.error(f"Error fetching customer names from Google Sheets: {str(e)}")
            return Response({
                'error': 'Failed to fetch customer names'
