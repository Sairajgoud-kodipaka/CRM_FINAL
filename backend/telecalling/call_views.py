...
from .models import CallRequest, Lead, AuditLog
...
class CallRequestViewSet(ModelViewSet):
    ...
    def initiate(self, request):
        ...
        serializer = CallInitiationSerializer(data=request.data)
        ...
        try:
            with transaction.atomic():
                ...
                if calls_today >= 3:  # Max 3 calls per lead per day
                    return Response({
                        'error': 'Rate limit exceeded',
                        'message': 'Maximum 3 calls per lead per day'
    ...
    def _initiate_exotel_call(self, call_request):
        ...
        try:
            ...
            # Exotel API endpoint
            url = f"https://api.exotel.com/v1/Accounts/{exotel_config['account_sid']}/Calls/connect.json"
            
            # Prepare call data
            call_data = {
                'From': exotel_config['agent_number'],  # Telecaller's number
                'To': call_request.lead.phone,  # Customer's number
                'CallerId': exotel_config['caller_id'],
                'Url': exotel_config['webhook_url'],
                'TimeLimit': 1800,  # 30 minutes max
                'Record': 'true' if exotel_config.get('record_calls', False) else 'false',
                'CustomField': json.dumps({
                    'lead_id': str(call_request.lead.id),
                    'call_request_id': str(call_request.id),
                    'telecaller_id': str(call_request.telecaller.id)
                })
            }
